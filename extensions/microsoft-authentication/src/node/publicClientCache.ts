...
export class CachedPublicClientApplicationManager implements ICachedPublicClientApplicationManager {
	...
	private async _doCreatePublicClientApplication(clientId: string) {
		const pca = new CachedPublicClientApplication(clientId, this._cloudName, this._secretStorage, this._logger);
